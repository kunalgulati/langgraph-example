...
def should_continue(state):
    ...
    if has_final_estimate:
        return "end"
    # If we've hit the max iterations or don't have tool calls, force an end
    elif len(messages) >= (MAX_TOOL_ITERATIONS * 2):  # *2 because each iteration has agent + action
        return "end"
    # If there are tool calls and we haven't hit the limit, continue
    elif has_tool_calls:
        return "continue"
    # Otherwise, we end to get a final estimate
    else:
        return "end"
